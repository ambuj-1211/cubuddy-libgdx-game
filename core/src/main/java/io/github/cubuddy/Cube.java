...
public class Cube {
    ...
    private void processKeys () {
        ...
        if ((Gdx.input.isKeyPressed(Input.Keys.SPACE) || controlButton) && state == FOLLOW && stateTime > 0.5f) {
            stateTime = 0;
            state = CONTROLLED;
            return;
        }

        if ((Gdx.input.isKeyPressed(Input.Keys.SPACE) || controlButton) && state == CONTROLLED && stateTime > 0.5f) {
            stateTime = 0;
            state = FIXED;
            return;
        }

        if ((Gdx.input.isKeyPressed(Input.Keys.SPACE) || controlButton) && state == FIXED && stateTime > 0.5f) {
            stateTime = 0;
            state = CONTROLLED;
            return;
        }

        if ((Gdx.input.isKeyPressed(Input.Keys.F) || followButton) && stateTime > 0.5f) {
            stateTime = 0;
            state = FOLLOW;
            return;
        }
